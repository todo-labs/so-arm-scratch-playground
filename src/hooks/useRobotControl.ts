    ...
    try {
      ...
      for (let i = 0; i < jointDetails.length; i++) {
        try {
          ...
          log.error(`Failed to initialize joint ${jointDetails[i].servoId}`, error);
          initialPos.push(0);
      ...
      alert(error);
      log.error("Failed to connect to the robot", error);
    }
    ...
    try {
      ...
      for (let i = 0; i < jointDetails.length; i++) {
        try {
          ...
          log.error(`Failed to reset joint ${jointDetails[i].servoId} during disconnect`, error);
        }
      }

      await scsServoSDK.disconnect();
      setIsConnected(false);
      log.info("Robot disconnected successfully");
    } catch (error) {
      log.error("Failed to disconnect from the robot", error);
    }
      ...
      if (jointIndex !== -1) {
        ...
        if (isConnected) {
          try {
            ...
            console.error(`Failed to update speed for joint with servoId ${servoId}:`, error);
      ...
      try {
        ...
        for (const joint of jointDetails) {
          try {
            ...
            log.error(`Failed to disable torque for joint ${joint.servoId}`, error);
          }
        }
        // Reset all joint states to "N/A"
        const newStates = jointStates.map((state, idx) => ({
          ...state,
          degrees: state.jointType === "revolute" ? (initialPositions[idx] ?? 0) : 0,
          speed: state.jointType === "continuous" ? 0 : undefined,
          targetDegrees: state.jointType === "revolute" ? (initialPositions[idx] ?? 0) : 0,
          isMoving: false,
        }));
        setJointStates(newStates);
        log.info("Emergency stop executed successfully");
      } catch (error) {
        log.error("Failed to execute emergency stop", error);
      }
    } else {
      log.warn("Cannot execute emergency stop, robot is not connected");
    }
  }, [isConnected, jointDetails, jointStates, initialPositions, scsServoSDK]);

  // Gripper control
  const openGripper = useCallback(async () => {
    await updateJointDegrees(SERVO_CONFIG.GRIPPER_SERVO_ID, SERVO_CONFIG.GRIPPER_OPEN_ANGLE);
  }, [updateJointDegrees]);

  const closeGripper = useCallback(async () => {
    await updateJointDegrees(SERVO_CONFIG.GRIPPER_SERVO_ID, SERVO_CONFIG.GRIPPER_CLOSE_ANGLE);
  }, [updateJointDegrees]);
